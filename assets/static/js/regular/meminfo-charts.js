probes.register({
  name: 'MemInfo',
  init: function() {
    this.pie_chart_obj = new google.visualization.PieChart(document.getElementById('memory_pie')); 
    this.area_chart_obj = new google.visualization.AreaChart(document.getElementById('memory_area'));    
  },
  paint: function() {
    var pie_chart_obj = this.pie_chart_obj;
    var area_chart_obj = this.area_chart_obj;
    ...
    $.ajax('/api/mem_info/30', { dataType: 'json' }).done(function(response) {
      ...
      area_chart_obj.draw(areaData, options);
    });
    ...
    $.ajax('/api/mem_info/1', { dataType: 'json' }).done(function(response) {
      ...
      pie_chart_obj.draw(pieData, options);
    }); 
  }
});
